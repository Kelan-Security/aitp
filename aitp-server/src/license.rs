//! Kernex Binary License Enforcement
//!
...
//
const KERNEX_LICENSE_PUBKEY_HEX: &str =
...
const LICENSE_SEARCH_PATHS: &[&str] = &[
    "/etc/kernex/kernex.license",
    "/opt/kernex/kernex.license",
    "./kernex.license",
    "./aitp-server/kernex.license",
];
...
/// The license file placed at /etc/kernex/kernex.license
...
pub struct LicenseFile {
    ...
    /// Hex-encoded. Generated by kernex-license-tool using your private key.
    pub signature:  String,
}

impl LicenseFile {
    /// The bytes that were signed — everything except the signature field.
    /// MUST match exactly what kernex-license-tool signs.
...
fn load_and_validate() -> anyhow::Result<ActiveLicense> {
    ...
    for path in LICENSE_SEARCH_PATHS {
        // Also check KERNEX_LICENSE_PATH env var
        let path = std::env::var("KERNEX_LICENSE_PATH")
...
pub fn validate_license(license: &LicenseFile) -> Result<(), LicenseError> {
    ...
    let pubkey_bytes = hex::decode(KERNEX_LICENSE_PUBKEY_HEX)
    ...
    if license.expires_at > 0 {
        ...
        if days_remaining <= 30 {
            tracing::warn!(
                "⚠ License expires in {} days ({}). Contact tanush@kernex.io to renew.",
                days_remaining,
                license.org_name
            );
        }
...
fn print_license_banner(license: &ActiveLicense) {
    ...
    if matches!(license.source, LicenseSource::Community) {
        ...
        println!("  Upgrade at kernex.io or email tanush@kernex.io");
        println!("  ─────────────────────────────────────────────────");
    }
}
            ...
            if now > license.expires_at {
                tracing::error!(
                    "LICENSE EXPIRED. Server will continue running but \
                     ...
                     Contact tanush@kernex.io to renew."
                );
                ...
                if days <= 7 {
                    tracing::warn!(
                        "⚠ License expires in {} days! Renew at kernex.io", days
                    );
                }
            }
        }
