...
impl LicenseTier {
    ...
    pub fn default_features(&self) -> Vec<LicenseFeature> {
        match self {
            ...
            LicenseTier::Startup => vec![LicenseFeature::Postgres, LicenseFeature::Webhooks],
...
impl ActiveLicense {
    ...
    pub fn get() -> &'static Self {
        ACTIVE_LICENSE
            .get()
            .expect("License not initialised — call init_license() at startup")
    }

    /// Check if a feature is enabled
    pub fn has_feature(&self, feature: &LicenseFeature) -> bool {
        self.tier.default_features().contains(feature) || self.features.contains(feature)
    }
...
fn load_and_validate() -> anyhow::Result<ActiveLicense> {
    ...
    for path in LICENSE_SEARCH_PATHS {
        ...
        let path = std::env::var("KELAN_LICENSE_PATH").unwrap_or_else(|_| path.to_string());
        ...
        if std::path::Path::new(&path).exists() {
            ...
            let license: LicenseFile =
                serde_json::from_str(&content).map_err(|e| LicenseError::InvalidFormat {
...
pub fn validate_license(license: &LicenseFile) -> Result<(), LicenseError> {
    ...
    let pubkey_bytes =
        hex::decode(KELAN_LICENSE_PUBKEY_HEX).map_err(|_| LicenseError::InvalidPublicKey)?;
    ...
    let verifying_key =
        VerifyingKey::from_bytes(&pubkey_arr).map_err(|_| LicenseError::InvalidPublicKey)?;

    // ── Step 2: Decode the signature from the license file ─────────────────
    let sig_bytes = hex::decode(&license.signature).map_err(|_| LicenseError::InvalidSignature)?;
    ...
    loop {
        ...
        if license.expires_at > 0 {
            ...
            if now > license.expires_at {
                ...
                if days <= 7 {
                    tracing::warn!("⚠ License expires in {} days! Renew at kelan.io", days);
                }
            }
        }
    }
