...
impl DbPool {
    ...
    pub async fn get_org_by_email(&self, email: &str) -> Result<Organisation, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query_as("SELECT * FROM organisations WHERE email = $1")
                    .bind(email)
                    .fetch_one(p)
                    .await
            }
    ...
    pub async fn get_org_by_id(&self, id: &str) -> Result<Organisation, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query_as("SELECT * FROM organisations WHERE id = $1")
                    .bind(id)
                    .fetch_one(p)
                    .await
            }
        ...
        match self {
            DbPool::Postgres(p) => {
                sqlx::query(sql_pg)
                    ...
                    .bind(&e.public_key)
                    .bind(&e.department)
                    .bind(e.clearance_level)
                    .bind(&e.allowed_intents)
                    .bind(e.trust_score_avg)
                    .bind(e.session_count)
                    .bind(e.blocked_count)
                    .bind(e.quarantined)
                    .bind(e.enrolled_at)
                    .execute(p)
                    .await?;
    ...
    pub async fn get_entities(&self, org_id: &str) -> Result<Vec<Entity>, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query_as("SELECT * FROM entities WHERE org_id = $1 ORDER BY enrolled_at DESC")
                    .bind(org_id)
                    .fetch_all(p)
                    .await
            }
    ...
    pub async fn quarantine_entity(&self, id: &str) -> Result<u64, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                let r = sqlx::query("UPDATE entities SET quarantined = 1 WHERE id = $1")
                    .bind(id)
                    .execute(p)
                    .await?;
                Ok(r.rows_affected())
    ...
    pub async fn release_entity(&self, id: &str) -> Result<u64, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                let r = sqlx::query("UPDATE entities SET quarantined = 0 WHERE id = $1")
                    .bind(id)
                    .execute(p)
                    .await?;
                Ok(r.rows_affected())
    ...
    pub async fn delete_entity(&self, org_id: &str, id: &str) -> Result<u64, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                let r = sqlx::query("DELETE FROM entities WHERE id = $1 AND org_id = $2")
                    .bind(id)
                    .bind(org_id)
                    .execute(p)
                    .await?;
    ...
    pub async fn update_entity_last_seen(&self, id: &str) -> Result<(), sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query("UPDATE entities SET last_seen = $1 WHERE id = $2")
                    .bind(now())
                    .bind(id)
                    .execute(p)
                    .await?;
    ...
    pub async fn increment_entity_session_count(&self, id: &str) -> Result<(), sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query("UPDATE entities SET session_count = session_count + 1 WHERE id = $1")
                    .bind(id)
                    .execute(p)
                    .await?;
            }
    ...
    pub async fn increment_entity_blocked_count(&self, id: &str) -> Result<(), sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query("UPDATE entities SET blocked_count = blocked_count + 1 WHERE id = $1")
                    .bind(id)
                    .execute(p)
                    .await?;
            }
        ...
        match self {
            DbPool::Postgres(p) => {
                sqlx::query(sql_pg)
                    .bind(&s.id)
                    .bind(&s.org_id)
                    .bind(&s.source_entity_id)
                    .bind(&s.dest_entity_id)
                    .bind(&s.intent)
                    .bind(s.trust_score)
                    .bind(&s.verdict)
                    .bind(&s.ai_reasoning)
                    .bind(s.ai_latency_ms)
                    .bind(&s.status)
                    .bind(s.bytes_tx)
                    .bind(s.bytes_rx)
                    .bind(&s.anomaly_flags)
                    .bind(s.started_at)
                    .execute(p)
                    .await?;
            }
    ...
    pub async fn get_recent_audit(
        ...
        match self {
            DbPool::Postgres(p) => {
                sqlx::query_as(
                    "SELECT * FROM audit_chain WHERE org_id = $1 ORDER BY created_at DESC LIMIT $2",
                )
                .bind(org_id)
                .bind(limit)
                .fetch_all(p)
    ...
    pub async fn get_policies(&self, org_id: &str) -> Result<Vec<CommPolicy>, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                sqlx::query_as(
                    "SELECT * FROM comm_policies WHERE org_id = $1 ORDER BY priority ASC",
                )
                .bind(org_id)
                .fetch_all(p)
                .await
        ...
        match self {
            DbPool::Postgres(pool) => {
                sqlx::query(sql_pg)
                    ...
                    .bind(&p.source_type)
                    .bind(&p.dest_type)
                    .bind(&p.allowed_intents)
                    .bind(p.max_sessions_per_hour)
                    .bind(p.require_clearance_match)
                    .bind(p.enabled)
                    .bind(p.priority)
                    .bind(p.created_at)
                    .execute(pool)
    ...
    pub async fn delete_policy(&self, id: &str, org_id: &str) -> Result<u64, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                let r = sqlx::query("DELETE FROM comm_policies WHERE id = $1 AND org_id = $2")
                    .bind(id)
                    .bind(org_id)
                    .execute(p)
                    .await?;
    ...
    pub async fn get_stats(&self, uptime_secs: u64) -> Result<StatsResp, sqlx::Error> {
        let active_sessions: i64 = match self {
            DbPool::Postgres(p) => {
                sqlx::query_scalar("SELECT count(*)::bigint FROM sessions WHERE status = 'active'")
                    .fetch_one(p)
                    .await
                    .unwrap_or(0)
            }
        ...
        let avg_trust: Option<f64> = match self {
            DbPool::Postgres(p) => {
                sqlx::query_scalar("SELECT avg(trust_score)::float8 FROM sessions")
                    .fetch_one(p)
                    .await
                    .unwrap_or(None)
            }
            DbPool::Sqlite(p) => sqlx::query_scalar("SELECT avg(trust_score) FROM sessions")
                .fetch_one(p)
                .await
                .unwrap_or(None),
        };
        ...
        let blocked_today: i64 = match self {
            DbPool::Postgres(p) => sqlx::query_scalar(
                "SELECT count(*)::bigint FROM sessions WHERE verdict = 'Deny' AND started_at >= $1",
            )
            .bind(today)
            .fetch_one(p)
            .await
        ...
        let entities_online: i64 = match self {
            DbPool::Postgres(p) => sqlx::query_scalar(
                "SELECT count(*)::bigint FROM entities WHERE last_seen > $1 AND quarantined = 0",
            )
            .bind(now() - 300)
            .fetch_one(p)
            .await
            .unwrap_or(0),
            DbPool::Sqlite(p) => sqlx::query_scalar(
                "SELECT count(*) FROM entities WHERE last_seen > ? AND quarantined = 0",
            )
            .bind(now() - 300)
            .fetch_one(p)
            .await
        ...
        let threats_today: i64 = match self {
            DbPool::Postgres(p) => sqlx::query_scalar(
                "SELECT count(*)::bigint FROM security_incidents WHERE detected_at >= $1",
            )
            .bind(today)
            .fetch_one(p)
            .await
            .unwrap_or(0),
            DbPool::Sqlite(p) => {
                sqlx::query_scalar("SELECT count(*) FROM security_incidents WHERE detected_at >= ?")
                    .bind(today)
                    .fetch_one(p)
                    .await
                    .unwrap_or(0)
    ...
    pub async fn get_all_baselines(
        ...
        for r in rows {
            baselines.push(crate::sentinel::EntityBaseline {
                ...
                intent_distribution: serde_json::from_str(&r.intent_distribution)
                    .unwrap_or_default(),
        ...
        match self {
            DbPool::Postgres(p) => {
                sqlx::query(sql_pg)
                    ...
                    .bind(&intents)
                    .bind(b.avg_trust_score)
                    .bind(&peers)
                    .bind(b.avg_payload_bytes)
                    .bind(&hours)
                    .bind(lc)
                    .bind(sc)
                    .bind(ts)
    ...
    pub async fn delete_old_anomalies(&self, cutoff: i64) -> Result<u64, sqlx::Error> {
        match self {
            DbPool::Postgres(p) => {
                let r = sqlx::query("DELETE FROM anomalies WHERE detected_at < $1")
                    .bind(cutoff)
                    .execute(p)
                    .await?;
                Ok(r.rows_affected())
            ...
            DbPool::Sqlite(p) => {
                let r = sqlx::query("DELETE FROM anomalies WHERE detected_at < ?")
                    .bind(cutoff)
                    .execute(p)
                    .await?;
                Ok(r.rows_affected())
    ...
    pub async fn get_crypto_stats(&self) -> Result<Vec<(String, i64)>, sqlx::Error> {
        let sql_pg =
            "SELECT crypto_algorithm, count(*)::bigint FROM entities GROUP BY crypto_algorithm";
        let sql_sq = "SELECT crypto_algorithm, count(*) FROM entities GROUP BY crypto_algorithm";
        match self {
            DbPool::Postgres(p) => sqlx::query_as(sql_pg).fetch_all(p).await,
            DbPool::Sqlite(p) => sqlx::query_as(sql_sq).fetch_all(p).await,
        }
    }
}
