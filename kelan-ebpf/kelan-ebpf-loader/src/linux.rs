...
impl EbpfLoader {
    ...
    pub fn load_and_attach(
        ...
        info!(
            "Loading eBPF XDP program onto interface: {}",
            interface
        );
        
        // Verify interface exists
        if !interface_exists(interface) {
            return Err(EbpfLoaderError::InterfaceNotFound(
                interface.to_string()
            ));
        }
    ...
    fn load_native(
        ...
        let mut bpf = Bpf::load_file(bpf_object_path)
            .map_err(|e| EbpfLoaderError::LoadError(
                e.to_string()
            ))?;
        
        let program: &mut Xdp = bpf
            .program_mut("kelan_xdp")
            .ok_or_else(|| EbpfLoaderError::LoadError(
                "kelan_xdp program not found in object".into()
            ))?
            .try_into()
            .map_err(|e: _| EbpfLoaderError::LoadError(
        ...
        info!(
            "✓ eBPF XDP program attached to interface {}",
            interface
        );
...
impl Drop for EbpfLoader {
    fn drop(&mut self) {
        if self.attached {
            info!(
                "Detaching eBPF XDP from interface {}",
                self.interface
            );
...
mod tests {
    ...
    fn test_loader_graceful_fallback_no_object() {
        ...
        let result = EbpfLoader::load_and_attach(
            "lo",
            &PathBuf::from("/nonexistent/path.o"),
        );
    ...
    fn test_permit_session_software_mode() {
        ...
        assert!(loader.permit_session(
            12345, 0x7f000001, 0x7f000001, 9999999999
        ).is_ok());
    }
