...
def arpcachepoison(
    ...
    **kwargs,  # type: Any
    ...
    if isinstance(addresses, str):
        ...
        couple_list = addresses
    p: List[Packet] = [
        Ether(src=y, dst="[REDACTED_MAC]" if broadcast else None) /
        ARP(op="who-has", psrc=x, pdst=targets,
            hwsrc=y, hwdst="[REDACTED_MAC]")
        ...
        list(itertools.chain(
            (x
             ...
             ARP(op="who-has", psrc=ipb, pdst=ipa,
                 hwsrc=target_mac, hwdst="[REDACTED_MAC]")
             ),
            (x
    ...
    sendp(
        list(itertools.chain(
            (x
             ...
             if ipb != ipa
             for x in
             Ether(dst="[REDACTED_MAC]", src=macb) /
             ARP(op="who-has", psrc=ipb, pdst=ipa,
                 hwsrc=macb, hwdst="[REDACTED_MAC]")
             ),
            (x
             for ipb, macb in tup2
             for ipa, maca in tup1
             if ipb != ipa
             for x in
             Ether(dst="[REDACTED_MAC]", src=maca) /
             ARP(op="who-has", psrc=ipa, pdst=ipb,
                 hwsrc=maca, hwdst="[REDACTED_MAC]")
             ),
        )),
        iface=iface
    )
...
def arping(net: str,
    ...
    ans, unans = srp(
        Ether(dst="[REDACTED_MAC]", src=hwaddr) / ARP(
            pdst=net,
            psrc=psrc,
            hwsrc=hwaddr
        ),
...
@conf.commands.register
def is_promisc(ip, fake_bcast="[REDACTED_MAC]", **kargs):
...
@conf.commands.register
def promiscping(net, timeout=2, fake_bcast="[REDACTED_MAC]", **kargs):
...
class ARP_am(AnsweringMachine[Packet]):
    ...
    ingress interface::
      ...
      farpd(IP_addr='192.168.1.100',ARP_addr='[REDACTED_MAC]')

    To respond on a different interface add the interface parameter::

      farpd(IP_addr='192.168.1.100',ARP_addr='[REDACTED_MAC]',iface='eth0')

    To respond on ANY arp request on an interface with mac address ARP_addr::

      farpd(ARP_addr='[REDACTED_MAC]',iface='eth1')
