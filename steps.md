# Kelan Security Getting Started Guide

This guide describes how to set up the Kelan Security ecosystem—including the **Intelligence Core (Server)** and the **Client Agent (Daemon)**—from scratch for a new developer or security team.
...
The server is the brain of Kelan Security. It handles identity, trust evaluation, and policy enforcement.
...
Kelan Security supports **SQLite** (for easy dev) and **PostgreSQL** (for production).
...
- **For PostgreSQL**:
  ...
  2. In `.env`, uncomment: `DATABASE_URL=postgres://kelan:kelan_dev@localhost:5432/kelan`

### Step 2.3: Development Certificates
Generate a self-signed certificate for HTTPS:
```bash
mkdir -p certs
openssl req -x509 -newkey rsa:4096 -keyout certs/key.pem -out certs/cert.pem -sha256 -days 365 -nodes -subj "/C=US/ST=State/L=City/O=Kelan Security/CN=localhost"
```
...
```bash
cp aitp-client/kelan-agent.toml.example local-agent.toml
```

### Step 3.3: Running the Agent
Run the agent in the foreground for testing:
```bash
./target/debug/kelan-agent --config local-agent.toml start
```
...
```bash
./target/debug/kelan-agent --config local-agent.toml enroll --server localhost:3000 --token [YOUR_ORG_TOKEN]
```
...
### Check Agent Status
```bash
./target/debug/kelan-agent --config local-agent.toml status
```

### Test AI Trust Evaluation
Simulate a connection to see how the Intelligence Core evaluates it:
```bash
./target/debug/kelan-agent --config local-agent.toml test api.internal:443
```
...
| Action | Command |
|--------|---------|
| **Install as Service** | `sudo ./kelan-agent install` |
| **Stop Daemon** | `./kelan-agent stop` |
| **Reset Identity** | `./kelan-agent reset-keys` |
